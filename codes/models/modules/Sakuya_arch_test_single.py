...
class LunaTokis(nn.Module):
    def __init__(self, nf=64, nframes=3, groups=8, front_RBs=5, back_RBs=10):
        ...
        self.feat_imnet = Siren(in_features=200, out_features=192, hidden_features=[64, 64, 64, 256], 
                  hidden_layers=3, outermost_linear=True)
        self.flow_imnet = Siren(in_features=192+192+6+1+2, out_features=4, hidden_features=[64, 64, 64, 256], 
                  hidden_layers=3, outermost_linear=True)
        self.encode_imnet = Siren(in_features=192*2+192*2+12, out_features=32, hidden_features=[64, 64, 64, 256, 256], 
    ...
    def decoding_test(self, times=None, scale=None):
        ...
        for c in range(len(times)):
            ...
            del pred_p1, pred_p2, pred_p3, q_feat1, q_feat2, q_feat3, q_feat4, q_inp1, q_inp2, pe_coord
            torch.cuda.empty_cache()
            # inp1 = inp[:, :qs1]
            # inp2 = inp[:, qs1:qs1+qs2]
            # inp3 = inp[:, qs1+qs2:]
            # pred1 = self.encode_imnet(inp1.view(bs * qs1, -1)).view(bs, qs1, -1)
            # pred2 = self.encode_imnet(inp2.view(bs * qs2, -1)).view(bs, qs2, -1)
            # pred3 = self.encode_imnet(inp3.view(bs * qs3, -1)).view(bs, qs3, -1)
            # pred = torch.cat([pred1, pred2, pred3], dim=1)
            # del pred1, pred2, pred3, q_feat1, q_feat2, q_feat3, q_feat4, q_inp1, q_inp2, pe_coord
            # torch.cuda.empty_cache()
