...
from models.modules.warplayer import warp, warpgrid
...
class LunaTokis(nn.Module):
    def __init__(self, nf=64, nframes=3, groups=8, front_RBs=5, back_RBs=10):
        ...
        self.feat_imnet = Siren(in_features=200, out_features=192, hidden_features=[64, 64, 64, 64, 256], 
                  hidden_layers=4, outermost_linear=True)
        self.flow_imnet = Siren(in_features=192+192+6+1+2, out_features=4, hidden_features=[64, 64, 64, 64, 256], 
                  hidden_layers=4, outermost_linear=True)
        self.encode_imnet = Siren(in_features=192*2+192*2+12, out_features=27, hidden_features=[64, 64, 64, 256, 256], 
                  hidden_layers=4, outermost_linear=True)
    ...
    def decoding(self, times=None, scale=None):
        ...
        for c in range(len(times)):
            ...
            HRfeat = HRfeat.permute(0, 2, 1).view(bs, 192, HH, WW)
            q_feat = F.grid_sample(
            ...
            # flow_pred = F.avg_pool2d(flow_pred, kernel_size=3, stride=1, padding=1)
            ...
            # pred = pred.permute(0, 2, 1).view(bs, 32, HH, WW)
    ...
    def decoding_test(self, times=None, scale=None):
        ...
        for c in range(len(times)):
            ...
            HRfeat = torch.cat([pred_p1, pred_p2], dim=1)
            del q_coord, rel_coord, inp, inp_p1, inp_p2, pred_p1, pred_p2
            ...
            flow_pred = torch.cat([flow_pred1, flow_pred2], dim=1)
            del q_feat, q_inp, q_feat0, flow_inp, flow_inp1, flow_inp2, flow_pred1, flow_pred2
            torch.cuda.empty_cache()
            flow_pred = flow_pred.permute(0, 2, 1).view(bs, 4, HH, WW)
            # flow_pred = F.avg_pool2d(flow_pred, kernel_size=3, stride=1, padding=1)
            ...
            pred = torch.cat([pred_p1, pred_p2], dim=1)
            del pred_p1, pred_p2, q_feat1, q_feat2, q_feat3, q_feat4, q_inp1, q_inp2, pe_coord
            torch.cuda.empty_cache()
            # pred = self.encode_imnet(inp.view(bs * qs, -1)).view(bs, qs, -1)
            # pred = pred.permute(0, 2, 1).view(bs, 32, HH, WW)
            # pred = self.final_conv(pred)
