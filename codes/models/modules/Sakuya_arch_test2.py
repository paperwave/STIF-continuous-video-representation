...
class LunaTokis(nn.Module):
    def __init__(self, nf=64, nframes=3, groups=8, front_RBs=5, back_RBs=10):
        ...
        self.feat_imnet = Siren(in_features=200, out_features=192, hidden_features=[64, 64, 256], 
                  hidden_layers=2, outermost_linear=True)
        self.flow_imnet = Siren(in_features=193+192+6, out_features=4, hidden_features=[64, 64, 256], 
                  hidden_layers=2, outermost_linear=True)
        self.encode_imnet = Siren(in_features=192*2+192*2+12, out_features=3, hidden_features=[64, 64, 256, 256], 
    ...
    def decoding_mulfeat(self, times, scale=None):
        preds = []
        for fid in range(3):
            feat = torch.cat([self.feat[:, fid*2], self.feat[:, fid*2+1],self.feat[:, fid*2+2]], dim=1)
            ...
            if fid == 2:
                times = [torch.tensor([0.]), torch.tensor([0.5]), torch.tensor([1.])]
                times = [t_[None].repeat(bs, 1).cuda() for t_ in times]
            else:
                times = [torch.tensor([0.]), torch.tensor([0.5])]
                times = [t_[None].repeat(bs, 1).cuda() for t_ in times]
            for c in range(len(times)):
                qs = coord_highres.shape[1]
                q_feat = F.grid_sample(
                ...
                inp = torch.cat([q_feat, rel_coord, pe_coord], dim=-1)
                HRfeat = self.feat_imnet(inp.view(bs * qs, -1)).view(bs, qs, -1)
                ...
                q_feat = F.grid_sample(
                    ...
                    mode='nearest', align_corners=False)[:, :, 0, :] \
                    .permute(0, 2, 1)
                flow_inp = torch.cat([q_feat, pe_coord], dim=-1)
                flow_pred = self.flow_imnet(flow_inp.view(bs * qs, -1)).view(bs, qs, -1)
                flow_pred = flow_pred.permute(0, 2, 1).view(bs, 4, HH, WW)
                ...
                inp = torch.cat([q_feat1, q_feat2], dim=-1)
                pred = self.encode_imnet(inp.view(bs * qs, -1)).view(bs, qs, -1)
                pred = pred.permute(0, 2, 1).view(bs, 3, HH, WW)
    ...
    def forward(self, x, times=None, scale=None):
        self.gen_feat(x)
        if times is None:
            # return self.decoding_mulfeat(times, scale)
            bp()
