...
class LunaTokis(nn.Module):
    def __init__(self, nf=64, nframes=3, groups=8, front_RBs=5, back_RBs=10):
        ...
        self.flow_imnet = Siren(in_features=201, out_features=4, hidden_features=[64, 64, 256], 
                  hidden_layers=2, outermost_linear=True)
        self.encode_imnet = Siren(in_features=396, out_features=3, hidden_features=[64, 64, 256, 256], 
    ...
    def decoding(self, times=None, scale=None):
        ...
        for c in range(len(times)):
            ...
            flow_pred = self.flow_imnet(inp.view(bs * qs, -1)).view(bs, qs, -1)
            del q_feat, q_inp, rel_coord, pe_coord
            ...
            inp = torch.cat([q_feat3, q_feat4, q_img1, q_img2], dim=-1)
