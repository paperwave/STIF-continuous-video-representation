...
class LunaTokis(nn.Module):
    def __init__(self, nf=64, nframes=3, groups=8, front_RBs=5, back_RBs=10):
        ...
        self.feat_imnet = Siren(in_features=195, out_features=64, hidden_features=[64, 64, 256], 
                  hidden_layers=2, outermost_linear=True)
        self.flow_imnet = Siren(in_features=65, out_features=4, hidden_features=[64, 64, 256], 
                  hidden_layers=2, outermost_linear=True)
        self.encode_imnet = Siren(in_features=128, out_features=3, hidden_features=[64, 64, 256, 256], 
    ...
    def decoding_mulfeat(self, times):
        preds = []
        for fid in range(3):
    ...
    def forward(self, x, times=None, scale=None, test=False):
        self.gen_feat(x)
        if times is None:
            return self.decoding_mulfeat(times)
        else:
            return self.decoding(times)
